...
from recipe.grpohistbeta.buffer import Buffer
...
def compute_advantage(
    ...
    if adv_estimator == AdvantageEstimator.GAE:
        ...
        data.batch["advantages"] = advantages
        data.batch["returns"] = returns
    elif adv_estimator == AdvantageEstimator.GRPOHIST:
        # Initialize the mask for GRPO calculation
        grpo_calculation_mask = data.batch["response_mask"]
        # print(data)
        # assert 0
        # Call compute_grpo_outcome_advantage with parameters matching its definition
        advantages, returns = core_algos.compute_grpohist_outcome_advantage(
            token_level_rewards=data.batch["token_level_rewards"],
            response_mask=grpo_calculation_mask,
            index=data.non_tensor_batch["index"],
            norm_adv_by_std_in_grpo=norm_adv_by_std_in_grpo,
        ...
        grpo_calculation_mask = data.batch["response_mask"]
        # Call compute_grpo_outcome_advantage with parameters matching its definition
        advantages, returns = core_algos.compute_abspo_outcome_advantage(
            token_level_rewards=data.batch["token_level_rewards"],
            response_mask=grpo_calculation_mask,
            index=data.non_tensor_batch["uid"],
            norm_adv_by_std_in_grpo=norm_adv_by_std_in_grpo,
...
class RayGRPOHistBetaTrainer:
    ...
    def fit(self):
        ...
        # load buffer
        # self.buffer = Buffer(self.config.trainer.default_local_dir, self.global_steps, self.config.trainer.save_freq)
        ...
        for epoch in range(self.current_epoch, self.config.trainer.total_epochs):
            # Update current_epoch for checkpoint saving
            ...
            for batch_dict in self.train_dataloader:
                ...
                with marked_timer("step", timing_raw):
                    ...
                    print("indices", [d['index'] for d in batch.non_tensor_batch["extra_info"]])
                ...
                #     # recompute old_log_probs # haechan
                #     with marked_timer("old_log_prob", timing_raw, color="blue"):
                #         old_log_prob = self.actor_rollout_wg.compute_log_prob(batch) # 각 ray thread마다 fsdp_worker.py에 있는 함수를 실행하고 여기서 합쳐진다. 따라서 여기에서 batch size에 해당하는 결과를 볼 수 있음.
