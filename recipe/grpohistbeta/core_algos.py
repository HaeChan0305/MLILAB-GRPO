        ...
        for i in range(bsz):
            ...
            if len(id2score[key]) == epoch:
                ...
                assert len(id2score[key][epoch]["reward"]) < config.history.rollout_n, (
                    f"Duplicate sample detected! key={key}, "
                    f"rewards_count={len(id2score[key][epoch]['reward'])}, "
                    f"rollout_n={config.history.rollout_n}, epoch={epoch}, step={step}. "
                    f"This may indicate a checkpoint/dataloader state mismatch."
                )
                id2score[key][epoch]["reward"].append(scores[i].item())

            else:
                assert 0, (
                    f"Epoch mismatch! key={key}, id2score_len={len(id2score[key])}, "
                    f"expected_epoch={epoch}, step={step}. "
                    f"len(train_dataset) % batch_size should be 0, or checkpoint state is corrupted."
                )
...
# NOTE(sgm): this implementation only consider outcome supervision, where the reward is a scalar.
@register_adv_est(AdvantageEstimator.ABSPO)  # or simply: @register_adv_est("grpo")
def compute_abspo_outcome_advantage(
    token_level_rewards: torch.Tensor,
    response_mask: torch.Tensor,
    index: np.ndarray,
    epsilon: float = 1e-6,
    ...
    with torch.no_grad():
        scores = torch.where(scores == 0, torch.tensor(-1.0), torch.tensor(1.0))
        scores = scores.unsqueeze(-1) * response_mask
